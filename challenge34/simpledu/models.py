...
class Course(Base):
    ...
    author = db.relationship('User', backref=db.backref('courses', passive_deletes=True, lazy='dynamic'), uselist=False)
...
class Live(Base):
    ...
    user = db.relationship('User', backref=db.backref('user', lazy='dynamic'), uselist=False)
    ...
    @property
    def url(self):
        return url_for('live.detail', live_name=self.name, live_user=self.user.username)
